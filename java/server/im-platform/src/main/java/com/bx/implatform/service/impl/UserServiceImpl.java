...
//            throw new GlobalException(ResultCode.PROGRAM_ERROR, "用户不存在");
    ...
    @Override
    public LoginVO refreshToken2(Long userId) {
    ...
    public void update(UserVO vo) {
        ...
        if (!user.getNickName().equals(vo.getNickName()) || !user.getHeadImageThumb().equals(vo.getHeadImageThumb())) {
            ...
            queryWrapper.lambda().eq(Friend::getFriendId, vo.getId());
        ...
        if (!user.getHeadImageThumb().equals(vo.getHeadImageThumb())) {
            List<GroupMember> members = groupMemberService.findByUserId(vo.getId());
